...
- 保存前に、同じタスクの既存レポートを確認する
  - 現在のブランチ名から推測される既存レポート（`ls tkhstmp/task_*_*.md | grep {タスク名の一部}`）を検索
  - 既存レポートが見つかり、かつ同日（YYYYMMDD）の場合は、そのファイルを更新する（新規セクションを追記）
  - 既存レポートが見つからない、または別日の場合は、新規ファイルを作成する
