---
name: capy-review-with-codex
description: codexを使ってソースコードのレビューを実施する。コード品質、テスト、ドキュメント、セキュリティなどの観点で確認
...
# Review with Codex / Codexによるレビュー

codexを使って実装されたコードのレビューを実施する。コード品質、テスト、セキュリティ、ドキュメントの整合性を確認する。
...
codex実行時は以下の対策を必ず実施すること:

1. **環境変数経由でプロンプトを渡す**
   ```bash
   CODEX_PROMPT='レビュー対象のコード内容とレビュー観点'
   codex exec --sandbox read-only -- "$CODEX_PROMPT"
   ```

2. **禁止事項**
   - プロンプトを直接コマンド引数に展開しない
   - `${}` や引数展開を使わない
   - シェルのメタ文字をエスケープせずに使わない

### 操作範囲の制限

- 作業ディレクトリより上位のディレクトリを対象にしない
- 絶対パスによる検索を禁止
- 参照系操作のみに限定（Read、Grep、Globのみ使用）
- git操作は参照のみ（diff、log、show等）
...
2. **codexによるレビュー**
   - レビュー対象のファイル内容を読み取る
   - レビュー観点を含むプロンプトを作成
   - セキュリティ対策を施してcodexを実行
   ```bash
   CODEX_PROMPT='[ファイル内容とレビュー観点]' bash -c 'codex exec --sandbox read-only -- "$CODEX_PROMPT"'
   ```
   - codexの出力結果を取得
