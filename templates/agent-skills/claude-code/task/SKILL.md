...
### Phase 2.5: レビュー（Phase 2を実施した場合のみ）

Phase 2で実装を行った場合、コードレビューを実施する。

- **判定基準**: Phase 2で`capy-code`が実行された場合
- **引き継ぎ情報**: タスク説明、ブランチ名、Phase 2の実装結果
- **subagent_type**: `capy-review`
- **注意**: Phase 2を実施していない場合（調査のみのタスク）は、このフェーズをスキップする
...
- **引き継ぎ情報**: タスク説明、Phase 1/Phase 2/Phase 2.5の実行結果の要約
- **subagent_type**: `capy-report`

## サブエージェント呼び出しのルール

- 各サブエージェントには、そのフェーズで必要な情報を過不足なく伝える
- Phase 1では、`capy-understand`（コードベース理解）と`capy-research`（外部情報調査）を並列実行できる
  - 両方必要な場合は並列に実行する
