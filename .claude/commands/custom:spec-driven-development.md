...
ref: https://zenn.dev/sosukesuzuki/articles/593903287631e9
---
...
- このフェーズで !`mkdir -p ./tkhstmp/specs`  を実行します
- `./tkhstmp/specs` 内にタスクの概要から適切な spec 名を考えて、その名前のディレクトリを作成します
    - たとえば、「記事コンポーネントを作成する」というタスクなら `./tkhstmp/specs/create-article-component` という名前のディレクトリを作成します
