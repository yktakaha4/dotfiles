...
- 元々の表に対して例を追記
- `1.27e+30` は `127の後に0が30続く` 数

| 記法 | 例 | Python | n=100の場合 | n=1000の場合 |
| -- | -- | -- | -- | -- |
| O(1) | 2値テスト | `1` | 1 | 1 |
| O(log n) | ソート済み配列の二分探索 | `math.log2(n)` | 6.64 | 9.96 |
| O(n) | 連結リストの線形サーチ | `n` | 100 | 1,000 |
| O(n log n) | クイックソート（平均的な条件で） | `n * math.log2(n)` | 664 | 996 |
| O(n^2) | バブルソート（平均的な条件で） | `n ** 2` | 10,000 | 1,000,000 |
| O(2^n) | 数値の因数分解、指数的な成長 | `2 ** n` | 1.27e+30 | 1.07e+301 |
| O(n!) | 力任せに解く巡回セールスマン問題 | `math.factorial(n)` | 9.33e+157 | 4.02e+2567 |
